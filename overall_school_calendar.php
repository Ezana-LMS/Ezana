...
if (isset($_POST['add_school_calendar'])) {
    ...
    if (!$error) {
        ...
        $stmt->execute();
        if ($stmt) {
            $success = "Educational Dates Added" && header("refresh:1; url=overall_school_calendar.php");
        } else {
...
if (isset($_GET['delete'])) {
    ...
    $stmt->close();
    if ($stmt) {
        $success = "Deleted" && header("refresh:1; url=overall_school_calendar.php");
    } else {
        $info = "Please Try Again Or Try Later";
    }
}
                                        ...
                                        <div class="row">
                                            <div class="col-12">
                                                ...
                                                <table id="example1" class="table table-bordered table-striped">
                                                    ...
                                                    <tbody>
                                                        <?php
                                                        $ret = "SELECT * FROM `ezanaLMS_Calendar` WHERE faculty_id = ''  ORDER BY `ezanaLMS_Calendar`.`semester_start` ASC   ";
                                                        $stmt = $mysqli->prepare($ret);
                                                        $stmt->execute(); //ok
                                                        $res = $stmt->get_result();
                                                        $cnt = 1;
